...
class SessionSelectorPage(StackPage):
    ...
    def update_ready_state(self):
        ...
        # Resuming a session was selected instead...
            ...
            # Load previous session, if we're ready...
            if ready:

                # Temporarily disconnect signals that would have resulted in this
                #  callback being infinitely invoked because we change the
                #  user interfaces' expert name or email address...
                ...
                # Set API key user interface from restored training session...
                api_key = self._application._training_session.get_api_key()
                login_page._api_key_entry.set_text(api_key)

                # Set host user interface from restored training session...
                host = self._application._training_session.get_host()
                login_page._host_entry.set_text(host)

                # Set port user interface from restored training session...
                port = self._application._training_session.get_port()
                login_page._port_entry.set_text(str(port))

                # Set TLS user interface flag from restored training session...
                tls = self._application._training_session.get_tls()
                login_page._use_tls_switch.set_active(tls)

                # Set expert name user interface from restored training session...
                expert_name = self._application._training_session.get_expert_name()
                self._name_entry.set_text(expert_name)

                # Set expert email user interface from restored training session...
                expert_email = self._application._training_session.get_expert_email()
                self._email_entry.set_text(expert_email)

                # Restore signals that check if the user interfaces' expert name
                #  or email address has changed...
                self._name_entry.connect('notify::text', self.on_name_or_email_text_change)
                self._email_entry.connect('notify::text', self.on_name_or_email_text_change)

                # Show path and some information about restored traiing session file in status bar...
                self._application_window.set_status(_(F'Restored training session of {self._application._training_session.get_total_examples()} data points from {self._application._training_session.get_total_songs_listened()} songs listened to...'))
